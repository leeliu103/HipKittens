...
template<int axis, bool assume_aligned,
         ...
         ducks::coord::tile COORD = coord<ST>,
         int N_THREADS = WARP_THREADS>
__device__ inline void load(ST& dst, const GL& src, const COORD& idx, const uint32_t* swizzled_offsets, int warp_id)
{
    using T = typename ST::dtype;
    ...
    constexpr int num_warps = N_THREADS / kittens::WARP_THREADS;
    const int warpid = warp_id % num_warps;
...
__device__ static inline void load(ST &dst, const GL &src, const COORD &idx, const uint32_t* swizzled_offsets, int warp_id) {
    load<2, false, ST, GL, COORD, WARP_THREADS>(dst, src, idx, swizzled_offsets, warp_id);
}
