...
struct g2s_wrapper_2d {
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, axis, args...>::value) {
            ...
            constexpr int SIZE = H*W*B*D*R*C*rt_height*rt_width;
            // initialize
            dtype *d_i, *d_o;
            ...
            // make descriptors
            using GL = typename kittens::gl<dtype, -1, -1, -1, rt_width*C*W>;
            static_assert(axis::value==0 || axis::value==1 || axis::value==2, "Axis must be 0, 1, or 2.");
            GL input  (d_i, (axis::value==0?H*rt_height:1)*B, (axis::value==1?H*rt_height:1)*D, (axis::value==2?H*rt_height:1)*R, nullptr);
            GL output (d_o, (axis::value==0?H*rt_height:1)*B, (axis::value==1?H*rt_height:1)*D, (axis::value==2?H*rt_height:1)*R, nullptr);
            // run kernel
            hipFuncSetAttribute(
            ...
            // check and cleanup
            this_result.result = validate(d_i, d_o, i_ref, o_ref, this_result.label, W*rt_width);
        }
        else {
