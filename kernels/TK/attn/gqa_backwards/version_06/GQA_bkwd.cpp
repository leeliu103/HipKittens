...
__global__ void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
    ...
    // Outer loop over query heads in the group  
    for (int q_head_in_group = 0; q_head_in_group < GROUP_SIZE - 1; ++q_head_in_group) {
        const int q_head_idx = kv_head_idx * GROUP_SIZE + q_head_in_group;
        ...
        for (int i = 0; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
            ...
            // dot slice 0
            {
                ...
                atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, i * 4, 0}, warpid);
                __builtin_amdgcn_s_setprio(0);
                __builtin_amdgcn_s_barrier();
                __builtin_amdgcn_sched_barrier(0);
            }
            ...
            // dot slice 1
            {
                ...
                mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
                swap_layout_and_transpose(dQ_i, dQ_i_T);
                atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, i * 4 + 1, 0}, warpid);
                __builtin_amdgcn_s_setprio(0);
                __builtin_amdgcn_s_barrier();
                __builtin_amdgcn_sched_barrier(0);
            }
            ...
            // dot slice 2
            {
                ...
                mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
                swap_layout_and_transpose(dQ_i, dQ_i_T);
                atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, i * 4 + 2, 0}, warpid);
                __builtin_amdgcn_s_setprio(0);
                __builtin_amdgcn_s_barrier();
                __builtin_amdgcn_sched_barrier(0);
            }
            ...
            // dot slice 3
            {
                ...
                mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
                swap_layout_and_transpose(dQ_i, dQ_i_T);
                atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, i * 4 + 3, 0}, warpid);
                __builtin_amdgcn_s_setprio(0);
                __builtin_amdgcn_s_barrier();
                __builtin_amdgcn_sched_barrier(0);
            }
        ...
        // dot slice 0
        {
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (num_steps - 1) * 4, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 1
        {
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (num_steps - 1) * 4 + 1, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 2
        {
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (num_steps - 1) * 4 + 2, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 3
        {
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, (num_steps - 1) * 4 + 3, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
    ...
    for (int i = 0; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
        ...
        // dot slice 0
        {
            ...
            load(L_smem[toc], g.L_vec, {batch_idx, first_q_head + GROUP_SIZE - 1, 0, i + 1});
            G::load<1, false>(Q_i_smem[toc][0], g.Q, {batch_idx, (i + 1) * 2, first_q_head + GROUP_SIZE - 1, 0});
            auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
            store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
            load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][0], {0, 0}));
            load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][0], {0, 0}));
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, i * 4, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        ...
        // dot slice 1
        {
            ...
            load(delta_smem[toc], g.delta_vec, {batch_idx, first_q_head + GROUP_SIZE - 1, 0, i + 1});
            G::load<1, false>(dO_i_smem[toc][0], g.dOg, {batch_idx, (i + 1) * 2, first_q_head + GROUP_SIZE - 1, 0});
            auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
            store(attn_i_smem_subtile, dP_ij_bf16_accum_row); // bank conflicts
            load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][0], {1, 0}));
            load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][0], {1, 0}));
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, i * 4 + 1, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        ...
        // dot slice 2
        {
            ...
            G::load<1, false>(Q_i_smem[toc][1], g.Q, {batch_idx, (i + 1) * 2 + 1, first_q_head + GROUP_SIZE - 1, 0});
            auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
            store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
            load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][1], {0, 0}));
            load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][1], {0, 0}));
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, i * 4 + 2, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        ...
        // dot slice 3
        {
            ...
            G::load<1, false>(dO_i_smem[toc][1], g.dOg, {batch_idx, (i + 1) * 2 + 1, first_q_head + GROUP_SIZE - 1, 0});
            auto attn_i_smem_subtile = subtile_inplace<WARP_SIZE_KV, DOT_SLICE_QO>(attn_i_smem, {warpid, 0});
            store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
            load(dO_i_col, subtile_inplace<DOT_SLICE_QO, D>(dO_i_smem[tic][1], {1, 0}));
            load(Q_i_col, subtile_inplace<DOT_SLICE_QO, D>(Q_i_smem[tic][1], {1, 0}));
            ...
            mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
            swap_layout_and_transpose(dQ_i, dQ_i_T);
            atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, i * 4 + 3, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
    ...
    // dot slice 0
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps - 1) * 4, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
    // dot slice 1
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps - 1) * 4 + 1, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
    // dot slice 2
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps - 1) * 4 + 2, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
    // dot slice 3
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps - 1) * 4 + 3, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
